...
export function SearchScreen({ context }: { context: AppScreenContext }) {
  ...
  if (selected) {
    const selectedAmount = amount ?? selected.availableAmounts[0] ?? 25;
    return (
      <View style={styles.stack}>
        <Button onPress={() => setSelected(null)} variant="quiet">
          Back
        </Button>
        ...
        <Card title="Merchant detail" eyebrow="Catalogue">
          ...
          <View style={styles.amountGrid}>
            {selected.availableAmounts.map((nextAmount) => {
              const active = selectedAmount === nextAmount;
              return (
                <Pressable
                  accessibilityRole="button"
                  accessibilityState={{ selected: active }}
                  key={nextAmount}
                  onPress={() => setAmount(nextAmount)}
          ...
          <Button
            onPress={() => context.goToCheckout({ merchant: selected, amount: selectedAmount })}
            variant="primary"
          >
            Purchase
          </Button>
    ...
    <View style={styles.stack}>
      <Card title="Search" eyebrow="Giftcard catalogue">
        <TextInput
          autoCapitalize="none"
          autoCorrect={false}
          onChangeText={catalogue.setQuery}
        ...
        <Text style={styles.muted}>
          {catalogue.loading ? 'Loading catalogue' : `${catalogue.filtered.length} merchant options`}
        </Text>
      </Card>

      {catalogue.filtered.map((merchant) => (
        <Pressable key={merchant.id} onPress={() => setSelected(merchant)}>
